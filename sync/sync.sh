...
copyTLSCertificates() {
    ...
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/certs/docasap_wildcard.crt /etc/pki/tls/certs/
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/certs/docasap_wildcard.key /etc/pki/tls/private/
}
copyIndexFile() {
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/apache/index.html /var/www/html/sync${ENVIRONMENT}.docasap.com/
}
copyComposer() {
    cd /usr/local/bin
...
copyApplicationConfigFiles() {
    ...
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/application/da_encKeys_passwds.php /var/www/html/${ENVIRONMENT}.docasap.com/public/lib/
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/application/environment.js /var/www/html/testing.docasap.com/public/l_js/
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/application/production2test.php /var/www/html/${ENVIRONMENT}.docasap.com/public/lib/
    sudo aws s3 cp s3://da-stg-major/cfg/sync/app/dbConnect.php /var/www/html/${ENVIRONMENT}.docasap.com/public/lib/
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/application/robots.txt /var/www/html/${ENVIRONMENT}.docasap.com/public/
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/application/htaccess /var/www/html/${ENVIRONMENT}.docasap.com/public/.htaccess
    sudo aws s3 cp s3://da-app-configuration/${ENVIRONMENT}/sync/application/cache_enable.ini /var/www/html/${ENVIRONMENT}.docasap.com/config/
}
settingPermissions() {
...
copySSlFiles
echo "Copying ini files and conf files from S3 for SYNC application"
copyPackageConfigurationFiles
echo "Copying certificates from S3"
copyTLSCertificates
echo "Copying html file from S3"
copyIndexFile
echo "Copying Composer files from S3 for SYNC application"
copyComposer
echo "Copying application config files from S3 for SYNC application"
copyApplicationConfigFiles
